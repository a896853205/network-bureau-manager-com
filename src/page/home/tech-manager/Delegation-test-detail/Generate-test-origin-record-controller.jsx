...
import {
  GENERATE_DELEGATION_RECORD_WORD,
  UPLOAD_WORD_FILE,
  SELECT_TECH_DELEGATION_RECORD,
  GET_FILE_URL,
  SAVE_TECH_DELEGATION_RECORD
  ...
  const handleUploadFile = async file => {
    if (handleBeforeUpload(file)) {
      ...
      const fileUrl = await proxyFileFetch(UPLOAD_WORD_FILE, {
        file: file.file,
        folderName: 'delegation/originRecord'
      });
    ...
    form.validateFields(async (err, value) => {
      if (enterpriseDelegationUuid) {
        if (!err) {
          ...
          const res = await proxyFetch(SAVE_TECH_DELEGATION_RECORD, value);
